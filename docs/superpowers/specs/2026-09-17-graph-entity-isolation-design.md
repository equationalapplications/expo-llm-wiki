...
Suppression detection needs one direction of `runAsync().changes` to be trustworthy, and that requirement must be written down rather than assumed:

- **Required:** an adapter MUST report `changes: 0` when an `INSERT … ON CONFLICT DO UPDATE` is suppressed by the conflict clause's `WHERE`. Without this, a suppressed write could report a nonzero count, and the repository would return success and publish an outbox event for a write it did not perform — violating the bullet above. The conditional SQL still protects the foreign row in that case, so the defect is misreported success, not disclosure; it is a correctness bug regardless.
- **Tolerated:** an adapter MAY under-report a *permitted* write as `changes: 0`. The disambiguating re-read exists precisely so that under-reporting degrades to one extra `SELECT` rather than a spurious ownership error.

This is not a new burden on adapter authors. `EdgeRepository.addIgnoreDuplicate` already treats `result.changes > 0` as authoritative proof that an `INSERT OR IGNORE` inserted, and the public `edgesWritten` count in `upsertGraph`'s result is derived from it. An adapter that over-reports already miscounts today; this requirement makes an existing, load-bearing dependency explicit instead of adding one. Earlier drafts of this section leaned the other way — treating `changes` as wholly unreliable — which was an overstatement in the opposite direction and left the over-reporting case unaddressed.
