...
**Chosen strategy: conditional append for default prompts, placeholder hydration for custom overrides.**

- Default prompt constants (`INGEST_SYSTEM_PROMPT`, `LIBRARIAN_SYSTEM_PROMPT`) do **not** contain
  `{{ontologyManifest}}` or `{{ontologyModeInstructions}}` placeholders. When `ontologyContext` is
  non-null, `PromptService` **appends** the ontology instruction block to the system prompt.
- Custom prompt templates may include `{{ontologyManifest}}` and/or `{{ontologyModeInstructions}}`.
  When those placeholders are present, `PromptService` **hydrates** them from `ontologyContext` and
  does **not** append a second copy of the instructions.
- When `ontologyContext` is null (Off mode), the resulting prompts must be **byte-identical** to the
  pre-feature default prompt constants (no placeholders replaced, no appendix appended).
...
| Modify | `src/repositories/EntryRepository.ts` | Update `upsert` to persist `okf_type` on entries |
| Create | `src/utils/ontology.ts` | `emptyManifest`, `normalizeTitleKey`, `resolveNodeType`, `validateManifest`, `mergeOntologyUpdates`, `validateInlineEdges` |
| Create | `src/services/OntologyService.ts` | Mode resolution, validation, cache, edge resolution |
| Modify | `src/services/PromptService.ts` | Ontology context hydration and conditional append |
...
| Modify | `src/prompts.ts` | Default ingest/librarian prompts (unchanged when Off) |
| Create | `src/prompts/ontology.ts` | Ontology prompt appendix builder (`buildOntologyPromptAppendix`) |
