...
**Status (2026-08-17, post-implementation doc audit):** Code review of the implemented READMEs surfaced six defects that this spec carried verbatim into the published docs: the §1c recipe (`body:` not in `IngestDocumentParams`; missing required `sourceHash: string`), the §1c SQL snippet (fabricated `distance`, `min_distance`, `rank_weight` columns and `ORDER BY rank_weight DESC` — source uses `depth` and `ORDER BY depth ASC, updated_at DESC LIMIT ?`), the §2 Graph traversal APIs example (`minConfidence`/`maxNodes` are the internal `NeighborhoodQueryOptions` names — the public `GraphTraversalOptions` uses `minTraversalConfidence`/`maxTraversalNodes`; undefined `anchorFactId` identifier), the §2 Deterministic graph seeding example (omitted the required third `SQLiteAdapter` argument), and a broken cross-link to an intra-page anchor that exists only in the root README. Corrected in the published docs on this branch via `2e4ab58` (packages/core/README.md) and `cd326a5` (README.md); 1087/1087 core tests pass and the post-fix doc snippets typecheck against `packages/core/src/types.ts` (pre-fix names fail with TS2353). The spec body intentionally retains the original (pre-correction) snippets as the planning-time target — future specs should grep source signatures before quoting them (see memory `feedback_verify-doc-sigs-against-source`). The drift guard added in `1636796` greps `EdgeRepository.ts` for structural CTE pillars only and did not catch the README drift; it is a fence-balance check, not a content-correctness check. No plan workspace under `.superpowers/sdd/2026-08-17-graphrag-readme-positioning/` existed — the spec was implemented via direct branch commits, so there is no scratch directory to remove.
