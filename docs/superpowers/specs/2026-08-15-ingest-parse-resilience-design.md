...
Consequence of skipping ownership: `findLatestSourceHash(entityId, sourceRef)` returns the most-recently-committed hash (the prior complete attempt's hash, or `null` on first attempt) and `sourceRefIndexRepo.findActiveByEntityAndHash(entityId, sourceHash)` returns `null` until a full success occurs. `hasChanged` keeps returning `true` on every subsequent run for this `(entityId, sourceRef, sourceHash)` until a full ingest succeeds. **Retries re-extract the whole document, not just the failed chunks.** Deterministic chunking reproduces the same chunk *set*, so every retry runs every chunk through the LLM again; dedup happens at write time — a previously-successful chunk's facts collide on normalized title with the live partial rows and are counted `skippedDuplicate`, not skipped upstream. The retry cost model is therefore **(chunks × retry runs) LLM calls** until a document heals; this recurring cost is a stated property of the design so it is instrumented deliberately rather than discovered later. When a retry achieves `failedChunks === 0`, normal `upsertGraphCore` runs and replaces **both** the prior complete attempt and any accumulated partial attempts in one transaction — yielding a single clean final state.
...
- **Unbounded retry accumulation**: partial commits dedup only by exact normalized title, so each retry of a still-failing document can insert paraphrased near-duplicates of the successful chunks; growth is bounded only by a fully-successful run. Each retry also costs a full-document re-extraction (§4.2 cost model: chunks × retry runs LLM calls). Deferred: needs a spec-level mechanism (e.g. an attempt counter), not a local fix.
