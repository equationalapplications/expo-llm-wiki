...
- `golden-v2/` — llm-wiki/2: `generated`, `verified` (multiple + bare-mapping), `sources` with per-entry `usage_window`, `usage_window` sibling, `status` (lifecycle), `stale_after` (one past, one future), footnote body, `Attested Computation` concept as generic fact, task with `status`/`execution_status` rename
